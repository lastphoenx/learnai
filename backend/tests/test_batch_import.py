...
@patch("app.tasks.batch_import.batch_import_task")
@patch("app.services.batch_import_job._redis_client")
def test_retry_batch_import_regenerates_done_unit(mock_redis_fn, mock_task, tmp_path):
    store: dict[str, str] = {}
    client = MagicMock()
    client.setex = lambda key, _ttl, value: store.update({key: value})
    client.get = lambda key: store.get(key)
    mock_redis_fn.return_value = client
    mock_task.delay.return_value = MagicMock(id="celery-regen-1")
    ...
    update_batch_import_job(batch_id, status="done", payload={"default_preset": "posten_compact"})
    ...
    job = retry_batch_import_units(db, user, batch_id, [0])

    assert job["status"] == "queued"
    assert job["units"][0]["generate_status"] == "regen_pending"
    assert job["units"][0]["unit_id"] == "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
    assert job["units"][0]["error"] is None
    mock_task.delay.assert_called_once_with(batch_id, str(user.id))
