...
def _user(email: str = "thomas@example.com") -> User:
    return User(
        id=uuid.uuid4(),
...
def test_login_email_survives_display_name_update(env_keys):
    ...
    _write_account_display_name(user, "Thomas")
    assert _login_email(user) == "thomas@example.com"
    assert _account_display_name(user) == "Thomas"
...
def test_admin_reset_password_with_email(env_keys):
    ...
    target = _user("child@example.com")
    db = object()
    admin_reset_password(
        db,
        actor,
