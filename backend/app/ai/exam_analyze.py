...
def _extract_exam_text(path: Path, content_type: str | None) -> str:
...
def analyze_exam_content(
    ...
    *,
    ...
    raw = _extract_exam_text(path, content_type)
    if len(raw.strip()) < 20:
    ...
    result = complete(prompt=prompt, provider=name, system=ANALYSIS_SYSTEM, model=None)
