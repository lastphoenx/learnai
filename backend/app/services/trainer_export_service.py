...
def import_trainer_json(db: Session, user: User, payload: dict) -> dict:
    ...
    for index, raw in enumerate(modules):
        repaired = repair_generated_module(dict(raw))
        mod = UnitModule(
            unit=new_unit,
            order_index=index,
            title_encrypted=encrypt_text_master(repaired["title"]),
            content_encrypted=encrypt_json(repaired["content"]),
            quiz_encrypted=encrypt_json(
                shuffle_quiz_block(repair_quiz_block(repaired.get("quiz") or {"questions": []}))
            ),
        )
        db.add(mod)
