...
def _http(exc: UnitError) -> HTTPException:
    mapping = {
        ...
        "not_released": status.HTTP_403_FORBIDDEN,
    }
...
@router.get("/batch-import/{batch_id}/quality")
def units_batch_import_quality(
    batch_id: str,
    user: User = Depends(get_app_user),
    db: Session = Depends(get_db),
):
...
@router.post("/batch-import/{batch_id}/retry", status_code=status.HTTP_202_ACCEPTED)
def units_batch_import_retry(
    batch_id: str,
    body: dict,
    user: User = Depends(get_app_user),
    db: Session = Depends(get_db),
...
@router.patch("/{unit_id}/learner-release")
def units_set_learner_release(
    unit_id: UUID,
    body: UnitLearnerReleaseRequest,
    user: User = Depends(get_app_user),
    db: Session = Depends(get_db),
):
    try:
        result = set_unit_learner_release(db, user, unit_id, released=body.released)
        db.commit()
        return result
    except UnitError as exc:
        db.rollback()
